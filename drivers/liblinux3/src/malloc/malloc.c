...
#define ACQUIRE_MALLOC_GLOBAL_LOCK()  kos_mutex_lock(&malloc_global_mutex);
#define RELEASE_MALLOC_GLOBAL_LOCK()  kos_mutex_unlock(&malloc_global_mutex);
...
#define PREACTION(M)  ( kos_mutex_lock(&(M)->lock))
#define POSTACTION(M) { kos_mutex_unlock(&(M)->lock); }
...
{
    return (kos_kern_free(ptr) != 0) ? 0 : -1;
}

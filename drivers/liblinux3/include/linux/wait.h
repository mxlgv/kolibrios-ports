...
do{                                                         \
    wait_queue_t __wait = {                                 \
        ...
        .evnt      = kos_create_event(NULL, MANUAL_DESTROY),     \
    };                                                      \
    unsigned long flags;                                    \
                                                            \
    ...
    for(;;){                                                \
        if (condition)                                      \
            break;                                          \
        kos_wait_eventTimeout(__wait.evnt, timeout);             \
    };                                                      \
    if (!list_empty(&__wait.task_list)) {                   \
        spin_lock_irqsave(&wq.lock, flags);                 \
...
void wake_up(wait_queue_head_t *q)
{
    ...
    if(curr != NULL)
    {
        if(!WARN_ON(curr->evnt.handle == 0))
        {
            kevent_t event = {0};
            event.code = -1;
            kos_raise_event(curr->evnt, 0, &event);
        }
    }

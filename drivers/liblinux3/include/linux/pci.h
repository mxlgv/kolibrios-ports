...
{
	*val = kos_pci_read8(dev->busnr, dev->devfn, where);
	return 1;
}
static inline int pci_read_config_word(const struct pci_dev *dev, int where, u16 *val)
{
	*val = kos_pci_read16(dev->busnr, dev->devfn, where);
	return 1;
}
static inline int pci_read_config_dword(const struct pci_dev *dev, int where,
					u32 *val)
{
	*val = kos_pci_read32(dev->busnr, dev->devfn, where);
	return 1;
}
static inline int pci_write_config_byte(const struct pci_dev *dev, int where, u8 val)
{
	kos_pci_write8(dev->busnr, dev->devfn, where, val);
	return 1;
}
static inline int pci_write_config_word(const struct pci_dev *dev, int where, u16 val)
{
	kos_pci_write16(dev->busnr, dev->devfn, where, val);
	return 1;
}
static inline int pci_write_config_dword(const struct pci_dev *dev, int where,
					 u32 val)
{
	kos_pci_write32(dev->busnr, dev->devfn, where, val);
	return 1;
}
#endif

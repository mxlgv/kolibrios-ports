...
//int pci_scan_filter(u32 id, u32 busnr, u32 devfn)
{
    ...
    if(vendor == 0x15AD )
    {
        class = kos_pci_read32(busnr, devfn, PCI_CLASS_REVISION);
        class >>= 16;
...
static void pci_read_bases(struct pci_dev *dev, unsigned int howmany, int rom)
{
    ...
    for(pos=0; pos < howmany; pos = next)
    {
        ...
        l = kos_pci_read32(dev->busnr, dev->devfn, reg);
        kos_pci_write32(dev->busnr, dev->devfn, reg, ~0);
        sz = kos_pci_read32(dev->busnr, dev->devfn, reg);
        kos_pci_write32(dev->busnr, dev->devfn, reg, l);
        ...
        if (is_64bit_memory(l))
        {
            ...
            lhi = kos_pci_read32(dev->busnr, dev->devfn, reg+4);
            kos_pci_write32(dev->busnr, dev->devfn, reg+4, ~0);
            szhi = kos_pci_read32(dev->busnr, dev->devfn, reg+4);
            kos_pci_write32(dev->busnr, dev->devfn, reg+4, lhi);
            ...
            else if (lhi)
            {
                ...
                kos_pci_write32(dev->busnr, dev->devfn, reg,
    ...
    if ( rom )
    {
        ...
        l = kos_pci_read32(dev->busnr, dev->devfn, rom);
        kos_pci_write32(dev->busnr, dev->devfn, rom, ~PCI_ROM_ADDRESS_ENABLE);
        sz = kos_pci_read32(dev->busnr, dev->devfn, rom);
        kos_pci_write32(dev->busnr, dev->devfn, rom, l);
...
static void pci_read_irq(struct pci_dev *dev)
{
    ...
    irq = kos_pci_read8(dev->busnr, dev->devfn, PCI_INTERRUPT_PIN);
    dev->pin = irq;
    if (irq)
        irq = kos_pci_read8(dev->busnr, dev->devfn, PCI_INTERRUPT_LINE);
    dev->irq = irq;
};
...
int pci_setup_device(struct pci_dev *dev)
{
    ...
    class = kos_pci_read32(dev->busnr, dev->devfn, PCI_CLASS_REVISION);
 ...
 //   pci_fixup_device(pci_fixup_early, dev);
    ...
    switch (dev->hdr_type)
    {
        case PCI_HEADER_TYPE_NORMAL:                /* standard header */
            ...
            dev->subsystem_vendor = kos_pci_read16(dev->busnr, dev->devfn,PCI_SUBSYSTEM_VENDOR_ID);
            dev->subsystem_device = kos_pci_read16(dev->busnr, dev->devfn, PCI_SUBSYSTEM_ID);
            ...
            if (class == PCI_CLASS_STORAGE_IDE)
            {
                ...
                progif = kos_pci_read8(dev->busnr, dev->devfn,PCI_CLASS_PROG);
                if ((progif & 1) == 0)
                {
        ...
        case PCI_HEADER_TYPE_CARDBUS:               /* CardBus bridge header */
                ...
                dev->subsystem_vendor = kos_pci_read16(dev->busnr,
                                                  dev->devfn,
                                                  PCI_CB_SUBSYSTEM_VENDOR_ID);

                dev->subsystem_device = kos_pci_read16(dev->busnr,
                                                  dev->devfn,
                                                  PCI_CB_SUBSYSTEM_ID);
                break;
...
static pci_dev_t* pci_scan_device(u32 busnr, int devfn)
{
    ...
    id = kos_pci_read32(busnr, devfn, PCI_VENDOR_ID);
  ...
  /* if( pci_scan_filter(id, busnr, devfn) == 0)
    ...
    hdr = kos_pci_read8(busnr, devfn, PCI_HEADER_TYPE);
                   ...
                   u8 pos, int cap, int *ttl)
{
    ...
    while ((*ttl)--) {
        pos = kos_pci_read8(bus, devfn, pos);
        if (pos < 0x40)
            break;
        pos &= ~3;
        id = kos_pci_read8(bus, devfn, pos + PCI_CAP_LIST_ID);
        if (id == 0xff)
            break;
        if (id == cap)
            return pos;
                    ...
                    unsigned int devfn, u8 hdr_type)
{
    ...
    status = kos_pci_read16(bus, devfn, PCI_STATUS);
...
//    raw_spin_lock_irqsave(&pci_lock, flags);
    *value = kos_pci_read8(bus->number, devfn, pos);
...
//    raw_spin_lock_irqsave(&pci_lock, flags);
    *value = kos_pci_read16(bus->number, devfn, pos);
//    raw_spin_unlock_irqrestore(&pci_lock, flags);
    return 0;
}
...
//    raw_spin_lock_irqsave(&pci_lock, flags);
    *value = kos_pci_read32(bus->number, devfn, pos);
//    raw_spin_unlock_irqrestore(&pci_lock, flags);
    return 0;
}
...
//    raw_spin_lock_irqsave(&pci_lock, flags);
    kos_pci_write32(bus->number, devfn,where, val);
//    raw_spin_unlock_irqrestore(&pci_lock, flags);
    return 0;
}
